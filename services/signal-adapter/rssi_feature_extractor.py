...
Adapted from vendor/ruview-temp/v1/src/sensing/feature_extractor.py for
signal-adapter integration.  The WifiSample dependency has been removed;
use extract_from_array() directly with numpy arrays, or pass plain objects
that expose .rssi_dbm and .timestamp attributes.
"""
...
class RssiFeatureExtractor:
    ...
    def extract(self, samples: list) -> RssiFeatures:
        """
        Extract features from a list of sample objects.

        Each sample must expose .rssi_dbm (float) and .timestamp (float)
        attributes.  Only the most recent ``window_seconds`` of data are used.
    ...
    def _trim_to_window(self, samples: list) -> list:
